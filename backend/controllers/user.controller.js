...
export const register = async(req, res) => {
    try {
        ...
        if(!fullname || !email || !password) return res.status(400).json({message:"All field are required", success:false});
...
export const login = async(req, res) => {
    try {
        ...
        if(!email || !password) return res.status(400).json({message:"All field are required", success:false});
